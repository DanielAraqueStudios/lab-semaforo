/*
 * SEMAFOOR_A_TEST.ino
 * Sketch de prueba para ESP32-S3 (SEMAFORO A)
 * - Mide HC-SR04
 ...
 *
 * Pines configurados para ESP A:
 * LED_ROJO = 1
 * LED_AMARILLO = 2
 * LED_VERDE = 42
 * TRIG_PIN = 40
 * ECHO_PIN = 39
 * BUZZER_PIN = 38 (opcional)
 */
    ...
    case S_GREEN:
      if (autoMode) {
        if (vehicleDetected && elapsed < DEFAULT_GREEN_MS) {
          // opcional: extender verde (no implementado complejo)
        }
      ...
      break;
    case S_YELLOW:
      if (elapsed >= YELLOW_MS) { currentState = S_RED; stateStart = millis(); }
      break;
    case S_RED:
      // pasar a ALL_RED cuando el otro complete (no hay otro en este test), simplificamos
      if (elapsed >= DEFAULT_GREEN_MS/2) { currentState = S_ALL_RED; stateStart = millis(); }
      break;
  }
  updateLEDs();
}

// Serial commands: 'g' = green, 'r' = red, 'a' = auto mode on, 'm' = manual mode, 's' = status
void processSerial() {
...
void printMenu() {
  Serial.println("---- SEMAforo A - TEST UART ----");
